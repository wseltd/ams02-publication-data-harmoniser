...
def _make_measurements(n: int = 4) -> list[Measurement]:
    ...
    for i in range(n):
        measurements.append(Measurement(
            ...
            stat_error_high=0.001 * (i + 1),
            stat_error_low=0.001 * (i + 1),
            sys_error_high=0.0005 * (i + 1),
            sys_error_low=0.0005 * (i + 1),
            species="PROTON",
            unit="GV",
            axis_type="rigidity",
        ))
...
def test_csv_numeric_precision_preserved(tmp_path):
    ...
    m = Measurement(
        ...
        stat_error_high=1.23456789e-5,
        sys_error_high=9.87654321e-6,
        species="PROTON",
        unit="GV",
        axis_type="rigidity",
    )
